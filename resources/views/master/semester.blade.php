...
@section('title', 'Manajemen Semester Akademik')
@section('breadcrumb')
...
<div class="glass-card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
        ...
        <span style="font-size:0.85rem; color:#6b7280;">Total {{ number_format($semesterList->total()) }} semester</span>
    </div>
                ...
                <tr style="border-bottom: 1px solid #e5e7eb;">
                    <td style="padding: 1rem 0;">
                        <div style="display: flex; align-items: center; gap: 0.75rem;">
                            ...
                            @endif
                            <span style="font-weight: 600;">{{ $semester->nama_semester }}</span>
                        </div>
                    </td>
                    <td style="padding: 1rem 0; color: #6b7280;">{{ $semester->tahun_ajaran }}</td>
                    ...
                    <td style="padding: 1rem 0; text-align: center; color: #6b7280; font-size: 0.85rem;">
                        {{ $semester->jadwal()->count() }} jadwal
                    </td>
                    <td style="padding: 1rem 0; text-align: center;">
                        <div style="display: flex; gap: 0.5rem; justify-content: center;">
                            @if (!$semester->is_active)
                                <form action="{{ route('semester.set-active', $semester->id) }}" method="POST" style="display: inline;">
                                    @csrf
                            ...
                            @endif
                            <a href="{{ route('semester.edit', $semester->id) }}" class="btn-kinetic" style="padding: 0.5rem; background: #F1F3F5; color: #000; text-decoration: none; font-size: 0.8rem; border-radius: 8px; cursor: pointer;">
                            ...
                            <form action="{{ route('semester.destroy', $semester->id) }}" method="POST" style="display: inline;">
                                ...
                                <button type="submit" class="btn-kinetic" style="padding: 0.5rem; background: #FEE2E2; color: #BA1A1A; border: none; font-size: 0.8rem; border-radius: 8px; cursor: pointer;"
                                    onclick="return confirm('Hapus semester {{ $semester->display_name }}?')">
