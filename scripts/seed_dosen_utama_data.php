...
echo "📊 Mengisi data absensi untuk Dosen Utama (user_id=8)...\n\n";

// Jadwal yang baru dibuat untuk user 8
$jadwalList = DB::table('jadwal')
    ->where('user_id', 8)
    ->get();

$semester = DB::table('semester_akademik')->where('id', 4)->first();
$startDate = Carbon::parse($semester->tanggal_mulai);
$endDate = Carbon::parse($semester->tanggal_selesai);

// Generate 16 meeting dates per jadwal
$meetingDates = [];
...
while (count($meetingDates) < 16 && $currentDate->lte($endDate)) {
    $dayOfWeekIso = $currentDate->dayOfWeekIso;
    foreach ($jadwalList as $jadwal) {
        $jadwalDayIso = $dayMap[$jadwal->hari] ?? null;
        if ($jadwalDayIso === $dayOfWeekIso && count($meetingDates) < 16) {
...
echo "📅 Generated " . count($meetingDates) . " meeting dates\n";
echo "   Range: {$meetingDates[0]} to " . end($meetingDates) . "\n\n";
...
foreach ($jadwalList as $jadwal) {
    ...
    echo "📚 Jadwal ID {$jadwal->id}: {$jadwal->mata_kuliah_id} - Kelas {$jadwal->kelas_id} ({$mahasiswaList->count()} mahasiswa)\n";
...
echo "\n✅ Total {$totalInserted} absensi records inserted for Dosen Utama.\n";
...
foreach ($jadwalList as $jadwal) {
    $count = DB::table('absensi')
        ->where('jadwal_id', $jadwal->id)
        ->count();
    echo "   Jadwal {$jadwal->id} (MK {$jadwal->mata_kuliah_id}): {$count} records\n";
}
