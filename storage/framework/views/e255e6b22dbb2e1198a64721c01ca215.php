...
<div class="stats-grid">
    <div class="glass-card" style="border-left: 4px solid var(--kinetic-yellow);">
        ...
        <div style="font-size: 2.5rem; font-weight: 800; margin: 0.5rem 0;"><?php echo e(number_format($hadirHariIni)); ?> <span style="font-size: 1rem; font-weight: 400; opacity: 0.6;">Tap</span></div>
    ...
    <div class="glass-card">
        ...
        <div style="font-size: 2.5rem; font-weight: 800; margin: 0.5rem 0;"><?php echo e(number_format($sesiAktif)); ?> <span style="font-size: 1rem; font-weight: 400; opacity: 0.6;">Jadwal</span></div>
    ...
    <div class="glass-card">
        ...
        <div style="font-size: 2.5rem; font-weight: 800; margin: 0.5rem 0;"><?php echo e(number_format($totalDeviceAktif)); ?></div>
    ...
    <?php if(auth()->user()?->role === 'dosen'): ?>
        <div class="glass-card" style="border-left: 4px solid #0066CC;">
            ...
            <div style="font-size: 1.4rem; font-weight: 800; margin: 0.7rem 0 0.4rem;"><?php echo e($activeSemester?->display_name ?? 'Belum ada semester'); ?></div>
            <div style="color: #0066CC; font-size: 0.8rem; font-weight: 700;"><?php echo e(number_format($dosenScheduleCount)); ?> jadwal pengampu</div>
        </div>
    <?php endif; ?>
</div>
    ...
    <div class="glass-card" style="margin-bottom: 2rem; background: linear-gradient(135deg, rgba(0,102,204,0.04), rgba(29,177,115,0.04)); border: 1px solid rgba(0,102,204,0.08);">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:1rem; flex-wrap:wrap; margin-bottom:1rem;">
            ...
            <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                <a href="<?php echo e(route('dosen-courses')); ?>" class="btn-kinetic" style="text-decoration:none; padding:0.65rem 0.9rem; font-size:0.82rem; background:#0066CC; box-shadow:none;">Mata Kuliah Saya</a>
                <a href="<?php echo e(route('dosen-courses')); ?>" class="btn-kinetic" style="text-decoration:none; padding:0.65rem 0.9rem; font-size:0.82rem; background:#F1F5F9; color:var(--primary-dark); box-shadow:none;">Buka Sesi Jadwal</a>
                <a href="<?php echo e(route('reports.index', ['semester_id' => $activeSemester?->id])); ?>" class="btn-kinetic" style="text-decoration:none; padding:0.65rem 0.9rem; font-size:0.82rem; background:#1DB173; box-shadow:none;">Laporan Semester</a>
            </div>
        </div>

        <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 0.9rem;">
            <?php $__empty_1 = true; $__currentLoopData = $dosenAssignedSchedules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div style="background:#fff; border:1px solid #e5e7eb; border-radius:16px; padding:1rem; box-shadow:0 10px 30px rgba(0,0,0,0.03);">
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:0.75rem; margin-bottom:0.75rem;">
                        <div>
                            <div style="font-size:0.72rem; font-weight:700; color:#0066CC; text-transform:uppercase; letter-spacing:0.08em;"><?php echo e($group['semester']); ?></div>
                            <div style="font-size:1rem; font-weight:800; color:var(--primary-dark); margin-top:0.25rem;"><?php echo e(number_format($group['total'])); ?> jadwal</div>
                        </div>
                        <div style="font-size:0.72rem; color:#6b7280; text-transform:uppercase; font-weight:700;">Ringkas</div>
                    </div>

                    <div style="display:flex; flex-direction:column; gap:0.65rem;">
                        <?php $__currentLoopData = $group['items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $jadwal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div style="padding:0.7rem 0.8rem; background:#f8fafc; border-radius:12px;">
                                <div style="font-weight:800; font-size:0.9rem;"><?php echo e($jadwal->mata_kuliah?->nama_mk ?? '-'); ?></div>
                                <div style="font-size:0.8rem; color:#6b7280; margin-top:0.2rem;"><?php echo e($jadwal->kelas?->nama_kelas ?? '-'); ?> · <?php echo e($jadwal->hari); ?> · <?php echo e(substr($jadwal->jam_mulai, 0, 5)); ?>-<?php echo e(substr($jadwal->jam_selesai, 0, 5)); ?></div>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>

                    <div style="display:flex; gap:0.5rem; flex-wrap:wrap; margin-top:0.9rem;">
                        <a href="<?php echo e(route('dosen-courses')); ?>" class="btn-kinetic" style="text-decoration:none; padding:0.55rem 0.8rem; font-size:0.8rem; box-shadow:none;">Lihat Semua</a>
                        <a href="<?php echo e(route('dosen-courses')); ?>" class="btn-kinetic" style="text-decoration:none; padding:0.55rem 0.8rem; font-size:0.8rem; background:#F1F5F9; color:var(--primary-dark); box-shadow:none;">Mulai Sesi</a>
                    </div>
                </div>
...
<div style="display: grid; grid-template-columns: 2fr 1fr; gap: 2rem;">
    ...
    <div class="glass-card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
            ...
            <a href="<?php echo e(route('monitoring')); ?>" style="color: var(--primary-blue-container); font-size: 0.8rem; font-weight: 700; text-decoration: none;">Lihat Semua <i class="fas fa-arrow-right"></i></a>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $latestAbsensi; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr>
                        <td>
                            <div style="font-weight: 700;"><?php echo e($item->mahasiswa->nama ?? '-'); ?></div>
                            <div style="font-size: 0.75rem; color: var(--text-muted);"><?php echo e($item->mahasiswa->nim ?? '-'); ?></div>
                        </td>
                        <td><?php echo e($item->waktu_tap); ?></td>
                        <td><?php echo e($item->jadwal->mata_kuliah->nama_mk ?? '-'); ?></td>
                        <td>
                            <span class="status-pill <?php echo e(($item->status ?? '') === 'Telat' ? 'status-late' : (($item->status ?? '') === 'Alpa' ? 'status-absent' : 'status-present')); ?>">
    ...
    <div class="glass-card" style="background: rgba(0, 30, 64, 0.02);">
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem;">
            <?php $__empty_1 = true; $__currentLoopData = $recentDevices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $device): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php if($device->is_active): ?>
                    <div style="padding: 1rem; background: #fff; border-radius: var(--radius-md); border-left: 3px solid #1DB173;">
                        <div style="font-size: 0.7rem; color: var(--text-muted);"><?php echo e($device->last_seen_at?->format('d M Y H:i:s') ?? '-'); ?></div>
                        <div style="font-weight: 700; font-size: 0.85rem;"><?php echo e($device->device_id); ?> Online</div>
                        <div style="font-size: 0.75rem;"><?php echo e($device->name ?? 'IoT Device'); ?></div>
                    </div>
                <?php else: ?>
                    <div style="padding: 1rem; background: #fff; border-radius: var(--radius-md); border-left: 3px solid #BA1A1A;">
                        <div style="font-size: 0.7rem; color: var(--text-muted);"><?php echo e($device->last_seen_at?->format('d M Y H:i:s') ?? '-'); ?></div>
                        <div style="font-weight: 700; font-size: 0.85rem;"><?php echo e($device->device_id); ?> Offline</div>
                        <div style="font-size: 0.75rem;"><?php echo e($device->name ?? 'IoT Device'); ?></div>
                    </div>
                <?php endif; ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
document.addEventListener('DOMContentLoaded', function() {
    const adminWeeklyChart = <?php echo json_encode($adminWeeklyChart ?? ['labels' => [], 'data' => []], 512) ?>;
    const adminIotChart = <?php echo json_encode($adminIotChart ?? ['labels' => [], 'data' => []], 512) ?>;
    const dosenClassChart = <?php echo json_encode($dosenClassChart ?? ['labels' => [], 'data' => []], 512) ?>;
    const dosenCourseChart = <?php echo json_encode($dosenCourseChart ?? ['labels' => [], 'data' => []], 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\IOT-Attendace\resources\views/dashboard.blade.php ENDPATH**/ ?>